...
data_file_dir = DATADIR / 'COBE-SST3'
...
np.savetxt(
    ...
    np.arange(1850, end_year + 1).reshape(-1, 1),
    fmt='%.16f',
    delimiter=","
)
for ensemble_member in range(n_ensemble):
    ...
    for year in range(start_year, end_year + 1):
        ...
        new_ensemble_values[year - start_year, 0] = np.mean(
            gridded_to_timeseries(data),
            axis=0
        )  # ensemble provides daily data
    # COBE-SST3 uses 1890 perturbations for years prior to 1890
    new_ensemble_values = np.append(
        np.kron(
            np.ones((40, 1)),
            new_ensemble_values[0, 0]
        ),
        new_ensemble_values
    ).reshape(
        -1, 1)
    np.savetxt(
        data_file_dir / "ensemble_time_series.csv",
        np.concatenate((output, new_ensemble_values), axis=1),
        fmt='%.16f',
        delimiter=","
    )
