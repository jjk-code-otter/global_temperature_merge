...
class CMDCClient:
    ...
    def get_order_info(self, order_id):
        ...
        if js.get("code") != "200":
            raise RuntimeError(f"Failed to query order: {js}")

        return js["data"]

    def wait_order(self, order_id):
        """轮询到订单完成"""
        print(f"Wait for order to be completed...")
        ...
        for i in range(self.max_poll):
            ...
            if status == 4:
                ...
                raise RuntimeError(f"Order processing failed: {msg}")
    ...
    def download_file(self, url, order_id):
        ...
        if "filename=" in cd:
            # compatible filename="xxx.zip"
            file_name = re.findall(r'filename="?([^";]+)"?', cd)
            if file_name:
                file_name = file_name[0]

        # fallback: Infer filename from URL
        if not file_name:
            file_name = url.split("/")[-1]
            # If there is no file extension, add .dat
            if "." not in file_name:
                file_name = f"{order_id}.dat"

        save_path = os.path.join(self.output_dir, file_name)

        # Write to file + download progress bar
            ...
            for chunk in r.iter_content(chunk_size=1024):
                if chunk:
                    ...
                    if total:
                        ...
                        print(f"Download progress: {percent}% ", end="")

        print(f"Download completed: {save_path}")
        return save_path

    def retrieve(self, params):
        """Main process: Create order -> Poll order status -> Download"""
