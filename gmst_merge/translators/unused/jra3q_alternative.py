...
from gmst_merge.useful_functions import (
    ...
    monthly_to_annual_timeseries, days_in_month
)
...
while True:
    ...
    file_name = "jra3q-ms-mn.anl_surf125.0_0_0.tmp2m-hgt-an-ll125-mn." + date_string + "0100_" + date_string + str(
        days_in_month(current_year)[current_month - 1]).lstrip('[').rstrip(']') + "18.nc"
    ...
    month_of_data = np.kron(month_of_data, np.ones((1, 2, 1)))[:, 1:-2,
                    :]  # convert to 288 latitudinal bands with equal thickness
    month_of_data = gridded_to_timeseries(
        month_of_data.reshape(month_of_data.shape[0], month_of_data.shape[1], 1)).reshape(-1, 1)
