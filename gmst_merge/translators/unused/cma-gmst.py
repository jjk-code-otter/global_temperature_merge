...
data_file_dir = DATADIR / 'CMA-GMST'

client = CMDCClient(
    user_id="I36qmCaRBufW3y3Il7djpHirUXcdJpR9Fz40DmiawNP%2BeQYQvYHRNw%3D%3D",
    output_dir=data_file_dir
)  # This is Bruce's user ID
mean = np.zeros((0, 1))
current_year = 1850
current_month = 1
    ...
    if not file_location.is_file():
        ...
        try:
            ...
            for current_month in range(1, 13):
                ...
                month_data = np.concatenate(
                    (month_data[:, 0, :], np.kron(month_data, np.ones((1, 2, 1))), month_data[:, -1, :]),
                    'axis=1')  # convert to 180 latitudinal bands with equal thickness
                month_data = gridded_to_timeseries(
                    month_data.reshape(month_data.shape[0], month_data.shape[1], 1)).reshape(-1, 1)
