...
for i in range(5):
    for j in range(200):
        ...
        # softcoded filename
        matches = sorted(Path(data_file_dir).glob(
            f'20crtaspadzens_tas_mon-gl-72x36_hadcrut5_observation_ens-{i + 1}_1850-*_image_{j + 1}.nc'))
        data_file = netCDF4.Dataset(matches[-1])
        data_file = np.transpose(np.ma.getdata(data_file.variables['tas']).data)
        if ensemble == []:
            ensemble = monthly_to_annual_timeseries(
                gridded_to_timeseries(
                    data_file.reshape(member.shape[0], member.shape[1], 1)).reshape(-1, 1),
                1850
            )
        else:
            ensemble = np.concatenate((ensemble, monthly_to_annual_timeseries(
                gridded_to_timeseries(member.reshape(member.shape[0], member.shape[1], 1)).reshape(-1, 1), 1850)),
                                      'axis=1')
